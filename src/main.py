...
async def root():
    """Health check simple"""
    return {"status": "online", "service": "MatterCenter", "version": "0.1.0"}
...
async def health():
    ...
    return {
        ...
        "devices": [
            {"node_id": device.node_id, "name": device.name, "online": device.is_online}
...
app.include_router(
    commissioning.router, prefix="/api/commissioning", tags=["commissioning"]
)
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "main:app", host="0.0.0.0", port=port, reload=debug  # Hot-reload en desarrollo
    )
